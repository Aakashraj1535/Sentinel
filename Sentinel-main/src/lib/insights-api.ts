...
export interface FinancialImpactSeverityBucket {
  total: number;
  count: number;
}

export interface FinancialImpactTopSupplier {
  supplier: string;
  total: number;
  count: number;
}

export interface FinancialImpactTrendWeek {
  week: string;
  total: number;
}
...
export interface FinancialImpactSummary {
  totalAtRisk: number;
  pricedExceptionCount: number;
  unpricedExceptionCount: number;
  bySeverity: Record<string, FinancialImpactSeverityBucket>;
  topSuppliers: FinancialImpactTopSupplier[];
  trend: FinancialImpactTrendWeek[];
}

export async function fetchFinancialImpactSummary(): Promise<FinancialImpactSummary> {
  const res = await fetch(`${API_BASE}/api/analytics/financial-impact`);
  if (!res.ok) throw new Error("Failed to load financial impact summary");
  return res.json();
}
