...
def financial_impact_summary(cur, weeks: int = 12) -> dict:
    """
    Aggregated financial exposure for the Financial Impact panel:
    total $ currently at risk, a breakdown by severity, the suppliers
    with the most $ exposure, and a weekly trend -- same
    overall/trend/categories shape as root_cause_breakdown() so the
    frontend chart component can be reused with minimal changes.

    'At risk' (open exposure) only counts Active/Escalated exceptions --
    a Resolved exception's financial outcome already happened. The
    weekly trend, though, includes all exceptions with a computed
    estimate regardless of current status, since it's answering "how
    much exposure did we detect over time", not "what's still open".
    """
    since = datetime.now(timezone.utc) - timedelta(weeks=weeks)

    cur.execute("""
        SELECT COALESCE(SUM(estimated_financial_impact), 0) as total,
               COUNT(*) FILTER (WHERE estimated_financial_impact IS NOT NULL) as priced_count,
               COUNT(*) FILTER (WHERE estimated_financial_impact IS NULL) as unpriced_count
        FROM exceptions WHERE status IN ('Active', 'Escalated')
    """)
    open_row = cur.fetchone()

    cur.execute("""
        SELECT severity, COALESCE(SUM(estimated_financial_impact), 0) as total, count(*) as c
        FROM exceptions
        WHERE estimated_financial_impact IS NOT NULL AND status IN ('Active', 'Escalated')
        GROUP BY severity
    """)
    by_severity = {r["severity"]: {"total": float(r["total"]), "count": r["c"]} for r in cur.fetchall()}
    for sev in ("Low", "Medium", "High"):
        by_severity.setdefault(sev, {"total": 0.0, "count": 0})

    cur.execute("""
        SELECT s.name as supplier_name, COALESCE(SUM(e.estimated_financial_impact), 0) as total,
               count(*) as c
        FROM exceptions e
        JOIN suppliers s ON s.id = e.supplier_id
        WHERE e.estimated_financial_impact IS NOT NULL AND e.status IN ('Active', 'Escalated')
        GROUP BY s.name
        ORDER BY total DESC
        LIMIT 5
    """)
    top_suppliers = [{"supplier": r["supplier_name"], "total": float(r["total"]), "count": r["c"]}
                      for r in cur.fetchall()]

    cur.execute("""
        SELECT date_trunc('week', detected_at) as week,
               COALESCE(SUM(estimated_financial_impact), 0) as total
        FROM exceptions
        WHERE detected_at >= %s AND estimated_financial_impact IS NOT NULL
        GROUP BY week
        ORDER BY week
    """, (since,))
    trend = [{"week": r["week"].date().isoformat(), "total": float(r["total"])} for r in cur.fetchall()]

    return {
        "totalAtRisk": float(open_row["total"]),
        "pricedExceptionCount": open_row["priced_count"],
        "unpricedExceptionCount": open_row["unpriced_count"],
