...
no dollar-sign formatting beyond a plain number.

Exception type: {exception['exception_type']}
Severity: {exception['severity']}
Supplier: {supplier_name}
Order value: ${breakdown['orderValue']:,.2f}
SLA breached: {"Yes" if breakdown['slaBreached'] else "No"}
Estimated impact: ${breakdown['estimatedImpact']:,.2f} ({breakdown['totalPct'] * 100:.0f}% of order value)
"""
    try:
        ...
        return (
            f"Estimated ${breakdown['estimatedImpact']:,.2f} at risk on a "
            f"${breakdown['orderValue']:,.2f} order from {supplier_name}, based on "
...
def estimate_financial_impact(exception_id: str) -> dict:
    ...
    """, (exception_id,
          f"Estimated ${breakdown['estimatedImpact']:,.2f} at risk "
          f"({breakdown['totalPct'] * 100:.0f}% of ${breakdown['orderValue']:,.2f} order value)."))
...
if __name__ == "__main__":
    ...
    if not sample:
        ...
        if result["estimated_impact"] is not None:
            print(f"Estimated impact: ${result['estimated_impact']:,.2f}")
