...
LangGraph Pipeline
-------------------
Chains the agents into one automatic flow:

    detect (batch, finds ALL new exceptions)
        -> for each new exception:
             retrieve -> resolve -> estimate_impact -> report   (LangGraph-orchestrated)
...
individual exception then flows through the 4-step LangGraph graph:
retrieve context -> resolve with the LLM -> estimate financial impact ->
generate report.

estimate_impact runs AFTER resolve (not before) because its SLA-breach
component depends on the exception's current status/timing, and because
it's cheap and deterministic (see app/financial_impact.py) so it doesn't
meaningfully add to the per-exception latency the LLM call already costs.
...
def node_estimate_impact(state: PipelineState) -> dict:
    result = estimate_financial_impact(state["exception_id"])
    return {"financial_impact": result}
...
def build_graph():
    ...
    graph.add_node("estimate_impact", node_estimate_impact)
    graph.add_node("generate_report", node_report)
    graph.set_entry_point("retrieve")
    graph.add_edge("retrieve", "resolve")
    graph.add_edge("resolve", "estimate_impact")
    graph.add_edge("estimate_impact", "generate_report")
